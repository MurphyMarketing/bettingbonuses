...
export type SeriesFormValues = { name: string; slug: string; sportId: string; intro: string; description: string };
...
export function SeriesForm({
  ...
  action: (prev: SeriesFormState, fd: FormData) => Promise<SeriesFormState>;
  ...
  return (
    <form action={formAction} className="flex flex-col gap-6">
      ...
      <div className="flex flex-col gap-1.5">
        <Label>Intro</Label>
        <RichTextEditor name="intro" defaultValue={values.intro} placeholder="Series hub intro…" />
      </div>

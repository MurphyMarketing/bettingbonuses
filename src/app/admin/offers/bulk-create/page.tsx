...
import { db } from '@/db';
import { brands, sports, eventSeries, events } from '@/db/schema';
...
export default async function BulkCreatePage() {
  const [brandRows, sportRows, seriesRows, eventRows] = await Promise.all([
    ...
    db.select({ id: events.id, name: events.name, startsAt: events.startsAt }).from(events).where(gte(events.startsAt, sql`now() - interval '30 days'`)).orderBy(asc(events.startsAt)),
  ]);
  ...
  return (
    <main className="mx-auto max-w-4xl p-8">
      <div className="mb-6">
        ...
        <p className="text-sm text-muted-foreground">Scaffold one draft offer per selected brand, tied to a sport, series, or event.</p>
      </div>
      ...
      <BulkCreateForm
        ...
        events={eventRows.map((e) => ({ value: String(e.id), label: `${e.name} (${fmtDate(e.startsAt)})` }))}

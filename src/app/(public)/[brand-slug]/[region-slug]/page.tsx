...
export async function generateStaticParams() {
  ...
  // product — invalid combos 404 rather than being prerendered. Post-Sprint L
  // this second-level position is brand × region only; events are single-level
  // (/[series-slug]/), so there is no instance branch here anymore.
  const rows = await db
...
export default async function BrandRegionPage({ params }: { params: Params }) {
  const { 'brand-slug': brandSlug, 'region-slug': regionSlug } = await params;
  const ctx = await getContext(brandSlug, regionSlug);
  if (!ctx) notFound(); // brand × region only; invalid combo -> 404
